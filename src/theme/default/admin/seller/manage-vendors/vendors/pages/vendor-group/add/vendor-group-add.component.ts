...
export class vendorGroupAddComponent implements OnInit, OnDestroy {
      ...
      this.vendorGroupSandbox.vendorGroupDetail$.subscribe((data) => {
        if (data) {
          ...
          data.vendorGroupCategory.forEach((dataVal) => {
            ...
            this.settingSandbox.getCategoryListResponse$.subscribe((val) => {
              val?.forEach((value) => {
                if (value.categoryId == dataVal.categoryId) {
                  dataVal.levels = value.levels
                }
              })
            })
  ...
  onSubmit() {
    this.submitted = true;
    if (
      !this.vendorGroupAddForm.valid || Number(this.vendorGroupAddForm.controls["GroupCommission"].value) >= 100 || this.selectedCategory.length == 0)
     {
      return;
    }
    const params: any = {};

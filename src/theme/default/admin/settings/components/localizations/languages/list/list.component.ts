...
//constant
import {badgeStatusMappings, removeEmptyKeys, filterFields, getCustomTable } from './list.constant';
...
export class LanguageListComponent implements OnInit,  OnDestroy {
  ...
  private regSubscribeEvents() {
    this.subscriptions.add( this.languageSandbox.languageDelete$.subscribe(_delete => {
  ...
  private routeSubscribe(): void {
    ...
    this.subscriptions.add(this.languageSandbox.languageAddLoaded$.subscribe(data => {
      if (data === true) {
        this.languageList();
      }
    }));

    this.subscriptions.add(this.languageSandbox.languageUpdateLoaded$.subscribe(data => {
      if (data === true) {
        this.languageList();
      }
    }));

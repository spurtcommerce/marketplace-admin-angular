...
export class LanguageAddComponent implements OnInit {
  ...
  private convertBase64(inputValue: any): void {
    ...
    myReader.onloadend = e => {
      this.postImageUrl = this.domsanitizer.bypassSecurityTrustUrl(myReader.result as string);
  ...
  private languageList() {
    ...
    if (this.languageInfo[0] !== null) {
      if (this.languageInfo[0] && this.languageInfo[0].name) {
        this.updateTitle = 1;
        const imagePath = this.languageInfo[0].imagePath;
      const image = this.languageInfo[0].image;
      if (imagePath && image) {
        this.postImageUrl = this.imageUrl + '?path=' + imagePath + '&name=' + image + '&width=160&height=150';
      } else {

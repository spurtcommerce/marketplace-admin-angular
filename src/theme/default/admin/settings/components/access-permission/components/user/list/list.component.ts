...
// constant 
import { badgeStatusMappings, filterFields, getCustomTable, removeEmptyKeys } from './list.constant';
...
export class UserListComponent implements OnInit, OnDestroy {
  ...
  dynamicColumnFields: any = structuredClone(getCustomTable());
  ...
  private getUserList() {
    ...
    this.subscriptions.add(this.service.userlist(param).subscribe((res) => {
    ...
    if (this.pagination) {
      ...
      this.subscriptions.add(this.service.userpagiantion(param).subscribe((res) => {
  ...
  private updateSpinner(value: string, isValid: boolean = false): void {
    if(this.hasOwnProperty(value)){
     this.spinner = isValid;
    }
  }
  ...
  private goToPermission(user) {
    ...
    this.router.navigate(['/settings/access-and-permission/permission'], { queryParams: { user: JSON.stringify(details),name:'user' } });
  }

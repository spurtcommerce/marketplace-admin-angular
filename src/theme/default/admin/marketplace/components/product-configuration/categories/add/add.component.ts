...
export class CategoryAddComponent implements OnInit, OnDestroy {
  ...
  industryList() {
    ...
    param.keyword = '';
    param.sortOrder = '';
    this.sandbox.IndustryList(param);
    this.sandbox.IndustryList$.subscribe(val => {
      if (val) {
  ...
  onCategoryChange(data: any) {
    ...
    const categoryBasedIndustry = this.industryListData.find(val => val.id === data.industryId);
    this.user.controls['industry'].setValue(categoryBasedIndustry?.id || null);
    this.cd.detectChanges();
  }

  onIndustryChange(data: any) {

    if (data === undefined) {
      this.categoriesList = [...this.totalCategories];
      return
    }
    if (!['', null, undefined].includes(data.id)) {
      const param: any = { industryId: data.id };
      this.sandbox.industryBasedCategory(param);
      this.subscriptions.push(
        this.sandbox.industryBasedCategory$.subscribe((value) => {
          this.categoriesList = [];
          this.user.controls['categoryComponent'].setValue(null);
          this.cd.detectChanges();
          if (value?.length > 0) {
  ...
  *
  ...
  onSubmits(user): void {
    ...
    if (user.value.industry) {
      this.param.industryId = user.value.industry;
    } else {
      this.param.industryId = 0;
    }
    ...
    if (this.categoryId) {
      ...
      this.sandbox.getUpdateCategoriesData$.subscribe(data => {
        if (data && data.status === 1) {
          this.router.navigate(['/vendors/product-config/categories/list'], { queryParams: this.queryParamsData });
        }
      });
    } else {
  ...
  setCategory(details) {
    ...
    this.user.controls['categoryComponent'].setValue(
      details.parentInt == 0 ? null : details.parentInt
    );
    this.user.controls['industry'].setValue(
      details.industryId == 0 ? null : details.industryId
    );

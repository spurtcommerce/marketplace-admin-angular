...
export class CategoriesEffect {
  ...
  ExportAllExcel$: Observable<Action> = createEffect(() => this.action$.pipe(
    ...
    IndustryList$: Observable<Action> = createEffect(() => this.action$.pipe(
      ...
      switchMap((state) => {
        return this.categoriesService.IndustryList(state).pipe(
          switchMap((list) => [
            new actions.IndustryListSuccessAction(list),
          ]),
          catchError((error) =>
    ...
    industryBasedCategory$: Observable<Action> = createEffect(() => this.action$.pipe(
      ofType(actions.ActionTypes.INDUSTRY_BASED_CATEGORY),
      map((action: actions.industryBasedCategoryAction) => action.payload),
      switchMap((state) => {
        return this.categoriesService.industryBasedCategory(state).pipe(
          switchMap((list) => [
            new actions.industryBasedCategorySuccessAction(list),
          ]),
          catchError((error) =>
            of(new actions.industryBasedCategoryFailAction(error))
          )
        );
      })
    ));
